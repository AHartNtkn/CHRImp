...
fn main() {
 ...
 for case in ["common","distinct","failure","common-wide"] {
  for n in [8,32,128] {
   let(source,query)=workload(case,n);
   for rep in 0..23 {
    let p=prepare(source,&query);
    let mut new=Some(p.new);let mut old=Some(p.old);let mut query=Some(p.query);
    ...
    if rep>=2 {
     println!("{case},{n},{},current,{:.3},{:.3},0,{:.3},{nd:.3}",rep-2,p.parse,p.newprep,p.astdrop);
     println!("{case},{n},{},older,{:.3},{:.3},{:.3},{:.3},{od:.3}",rep-2,p.parse,p.oldprep,p.convert,p.astdrop);
    }
   }
  }
 }
