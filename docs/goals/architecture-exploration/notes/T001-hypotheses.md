...
## Ranking after direct-normalization measurements

The measured representation transformation advances: see T005-comparison.md. Selective joint/concrete execution is deferred. Its allocation witness requires equally specialized kernels on both sides to isolate grouping; otherwise a positive result could simply repeat the now-established benefit of source-derived operations. The statically certified boundary also may favor compiler-selected representation without establishing a dynamic grouping architecture. Reopen this comparison only with evidence of substantial conditional identity/liveness costs after useful specialization and a credible boundary that preserves common computation. This deferral does not establish that joint representation is universally preferable.

The combination review found no consequential overlap on the actual evaluator: each remaining rule has a connected constructor/control pair. The static whole-head feasibility pass would repeat consumer lookup there. Conversely the whole-head witness has no recognized normalization subsystem. A joint benchmark made from independent copies would establish coexistence, not interaction. No combination implementation is warranted by these cases. Future conditional feasibility would need shared-support intersection and invalidation after equality/consumption; individually nonempty components under opposing alternatives cannot join.

The later difficult-W checkpoint supplies a concrete reason to revisit conditional foundations: collection occupies 13.84 million of 18.64 million iterations after direct normalization, largely including global coordinate substitution. Counts do not establish CPU proportions, but show substantial repeated representation maintenance. Targeted source studies now compare ways to avoid rebuilding while preserving computational sharing and long-running release; no replacement has yet been selected.
