...
Advance or reject a candidate only after the loop’s post-spike diagnostic comparison establishes the architectural mechanism, its applicability and scaling, and the work or storage costs it introduces. An unmeasured mechanism remains unassessed. Do not polish a weak candidate through small optimizations or manufacture a verdict from runtime ratios. Fix correctness and experiment defects needed for the causal comparison, then reassess its architectural promise.
