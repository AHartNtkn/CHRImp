...
fn owned_collection_freezes_metadata_and_pending_updates_until_drop() {
    ...
    for cutoff in 0..48 {
        ...
        let staged = update.roots()[1];
        let mut gc = begin(&mut g, update.roots());
        let mut foreign = graph();
        assert!(catch_unwind(AssertUnwindSafe(|| gc.tick(&mut foreign))).is_err());
        assert!(catch_unwind(AssertUnwindSafe(|| g.collect([staged].into_iter()))).is_err());
        for _ in 0..cutoff {
            gc.tick(&mut g);
        }
        assert_eq!(g.fact(staged, id).unwrap().args, [3, 8]);
        ...
        assert_eq!(
            g.port(root, 0, 1, 8).unwrap().next(&g),
            Some((id, Condition::TRUE))
        );
