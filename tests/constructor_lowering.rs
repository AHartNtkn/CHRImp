...
fn start(p: &Program, q: &str) -> Engine {
    Engine::new(Arc::new(prepare(p, &parse_query(q).unwrap()).unwrap()))
}
...
fn recognition_is_invariant_under_source_renaming_and_order() {
    ...
    let mut e = start(&p, "renamed_app(R,A,B),renamed_app(R,C,D)");
    let a = answers(&mut e);
    assert_eq!(a.len(), 1);
    assert_eq!(variable(&e, &a[0], "A"), variable(&e, &a[0], "C"));
    assert_eq!(variable(&e, &a[0], "B"), variable(&e, &a[0], "D"));
    assert_eq!(a[0].rows.len(), 1);
    let mut e = start(&p, "renamed_nil(Sp),renamed_k(T),renamed_eval(T,Sp,O)");
    let a = answers(&mut e);
    assert_eq!(a.len(), 1);
...
fn source_normalization_is_shared_across_independent_choices() {
    let mut e = start(&behavior(), "(true;true),(true;true),app(R,A,B),app(R,C,D)");
    let a = answers(&mut e);
    assert_eq!(a.len(), 4);
...
fn finite_sibling_progresses_beside_actual_divergent_evaluator() {
    let mut e = start(&behavior(), "(app(T,T,X),eval(T,S,O),nil(S);k(K))");
...
fn malformed_consistency_cannot_silently_admit_a_smaller_family() {
    ...
    assert_eq!(rows(&e, &all[0], "app").len(), 1);
    assert_eq!(e.normalization_stats().transactions, 0);
}
#[test]
...
fn repeated_conditional_and_cyclic_unions_agree_with_source_execution() {
    ...
    let expected: serde_json::Value = serde_json::from_str(r#"[[[[0, 1, 1, 0, 1, 0, 1], [["app", [0, 1, 0]]]]], [[[0, 1, 2, 0, 1, 2, 0, 1, 2], [["app", [0, 1, 2]]]], [[0, 1, 2, 0, 1, 2, 0, 1, 2], [["app", [0, 1, 2]]]]], [[[0, 1, 2, 0, 1, 2], [["app", [0, 1, 2]]]], [[0, 1, 2, 0, 1, 2], [["app", [0, 1, 2]]]], [[0, 1, 2, 3, 1, 5], [["app", [0, 1, 2]], ["app", [3, 1, 5]]]], [[0, 1, 2, 3, 4, 2], [["app", [0, 1, 2]], ["app", [3, 4, 2]]]]], [[[0, 1, 2, 3, 4, 0, 1, 2], [["app", [0, 1, 2]]]], [[0, 1, 2, 3, 4, 0, 3, 4], [["app", [0, 3, 4]]]], [[0, 1, 2, 3, 4, 5, 1, 7], [["app", [0, 1, 2]], ["app", [5, 1, 7]]]], [[0, 1, 2, 3, 4, 5, 1, 7], [["app", [0, 1, 2]], ["k", [5]]]], [[0, 1, 2, 3, 4, 5, 1, 7], [["app", [0, 3, 4]], ["app", [5, 1, 7]]]], [[0, 1, 2, 3, 4, 5, 1, 7], [["app", [0, 3, 4]], ["k", [5]]]]], [[[0, 0, 2], [["k", [0]], ["k", [2]]]], [[0, 1, 0], [["k", [0]], ["k", [1]]]]]]"#).unwrap();
    for (case, q) in [
    ...
    .enumerate()
    {
        ...
        let mut actual: Vec<_> = answers(&mut e)
            ...
            .collect();
        actual.sort();
        assert_eq!(serde_json::to_value(actual).unwrap(), expected[case], "{q}");
    }
}
#[test]
fn unrestricted_identity_synthesis_runs_the_actual_evaluator() {
    ...
    let mut e = Engine::new(Arc::new(prepare(&behavior(), &q).unwrap()));
    let mut reader = Reader::default();
    let mut found = None;
...
fn known_dispatch_keeps_the_original_leading_post() {
    ...
    let mut e = start(&p, "k(T),choose(T)");
    let all = answers(&mut e);
    assert_eq!(all.len(), 1);
...
fn dispatch_runtime_chain_matches_parent_partition_oracles() {
    ...
    for (prefix, n, expected) in [
        ...
        let mut e = start(&p, &chain_query(prefix, n));
        let all = answers(&mut e);
...
fn dispatch_preserves_independent_fresh_choice_products() {
    ...
    let mut e = start(&p, &chain_query("", 4));
    let all = answers(&mut e);
...
fn dispatch_preserves_duplicate_arms_and_rejects_only_supported_clashes() {
    ...
    let mut e = start(&p, "nil(T),choose(T)");
...
fn dispatch_retains_conditional_field_equalities_without_cross_partition_merges() {
    ...
    let mut e = start(&p, "app(R,X,Y),app(S,Z,W),(T=R;T=S),route(T)");
    let all = answers(&mut e);
    assert_eq!(all.len(), 2);
    let mut sides = vec![];
    for a in &all {
...
fn dispatch_preserves_terminal_no_c_consumer_failure() {
    ...
    let mut e = start(&p, "no_c(T),(constant(T,D);k(T)),route(T)");
    let all = answers(&mut e);
    assert_eq!(all.len(), 1);
    let a = &all[0];
...
fn selected_original_fields_can_still_make_the_known_arm_fail() {
    ...
    let mut e = start(&p, "app(T,X,Y),k(X),s(Y),route(T)");
    assert!(answers(&mut e).is_empty());
